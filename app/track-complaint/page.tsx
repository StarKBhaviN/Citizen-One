...
export default function TrackComplaintPage() {
  ...
  // Track complaint function that uses the API
  const trackComplaintFunc = async (trackingId: string) => {
    if (!trackingId) {
    ...
    try {
      ...
      if (response) {  
        ...
        setError(""); // Clear any errors as we have valid data
      } else {
        setComplaint(null);
      ...
      setError(
        ...
        "Failed to track complaint. Please try again later."
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (initialId) {
      trackComplaintFunc(initialId);
    }
  }, [initialId]);

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    trackComplaintFunc(complaintId);
  };
  ...
  // Make sure timeline exists before rendering
  const hasTimeline = complaint && Array.isArray(complaint.timeline) && complaint.timeline.length > 0;
        ...
        <div className="container max-w-3xl">
          ...
          <Card className="mb-8">
            <CardHeader>
              ...
              <CardDescription>
                Enter the Complaint ID you received after submitting your
                complaint
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <CardContent>
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <p>{complaint.assignedTo || "Unknown"}</p>
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-500 dark:text-gray-400">
                        Priority
                      ...
                      {complaint.timeline.map(
                        (event: any, index: number) => (
                          <li key={event.id} className="mb-10 ml-6 last:mb-0">
                            <span
                              className={`absolute -left-3 flex h-6 w-6 items-center justify-center rounded-full ${getStatusColor(
                                event.status
                ...
                <CardContent>
                  <Accordion type="single" collapsible>
                    <AccordionItem value="faq-1">
                      ...
                      <AccordionContent>
                        The resolution time depends on the nature and complexity
                        of your complaint. For water supply issues, the typical
                        resolution time is 2-3 days. You can see the estimated
                        resolution date in the complaint details.
                      </AccordionContent>
                    </AccordionItem>
                    <AccordionItem value="faq-2">
                      ...
                      <AccordionContent>
                        Yes, you can provide additional information or updates
                        to your complaint by contacting our support team with
                        your Complaint ID.
                      </AccordionContent>
                    </AccordionItem>
                    <AccordionItem value="faq-3">
                      <AccordionTrigger>
                        What if my issue is not resolved by the estimated date?
                      </AccordionTrigger>
                      <AccordionContent>
                        If your issue is not resolved by the estimated date, it
                        will be automatically escalated to the department
                        supervisor. You can also contact our support team for
                        assistance.
                      </AccordionContent>
                    </AccordionItem>
                  </Accordion>
